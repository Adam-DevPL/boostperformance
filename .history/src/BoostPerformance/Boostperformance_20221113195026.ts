  ...
  public calculateInThreads = <T>(
    ...
    if (cluster.isPrimary) {
      ...
      tasks.forEach((task, index) => {
        this.workers[0].send(index)
        this.workers[0].on("message", (message: any) => {
        results.push(message)
        if (results.length === tasks.length) console.log(whatDoWithCalculations(results))
        workerIndex ++
        if (workerIndex === 3) workerIndex = 0
      })})
